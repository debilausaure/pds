...
int main (int argc, char *argv[]) {
	...
	char **path_directories = get_path_dirs();
...
int which(const char *command, const char **path_directories) {
...
char **get_path_dirs() {
	...
	for (char *path_char_ptr = path; *path_char_ptr != NULL; path_char_ptr++) {
	...
	char **path_dirs = malloc((dir_count + 1) * sizeof(char *));
	assert(path_dirs != NULL);
